...
impl Env {
    ...
    fn exec_into_new_pid_ns(&mut self, chroot_exec_file: PathBuf) -> Result<(), JailerError> {
        ...
        //
        // clawdbox (i.e. the "init" process of the new PID namespace) sets up handlers for some
        // signals including SIGHUP and jailer exits soon after spawning clawdbox into a new PID
        // namespace. If the jailer process is a session leader and its exit happens after
        // clawdbox configures the signal handlers, SIGHUP will be sent to clawdbox and be
        // caught by the handler unexpectedly.
        //
        // In order to avoid the above issue, if jailer is a session leader, creates a new session
        // and makes the child process (i.e. clawdbox) become the leader of the new session to
    ...
    fn copy_exec_to_chroot(&mut self) -> Result<OsString, JailerError> {
        ...
        //    clawdbox binary (like the executable .text section), this latter part is not
        //    desirable in clawdbox's threat model. Copying prevents 2 clawdbox processes from
        //    sharing memory.
...
mod tests {
    ...
    fn test_validate_exec_file() {
        ...
        std::fs::create_dir_all("/tmp/clawdbox_test_dir").unwrap();
        assert_eq!(
            format!(
                "{}",
                Env::validate_exec_file("/tmp/clawdbox_test_dir").unwrap_err()
            ),
            "/tmp/clawdbox_test_dir is not a file"
        );

        std::fs::remove_dir_all("/tmp/clawdbox_test_dir").unwrap();
    }
