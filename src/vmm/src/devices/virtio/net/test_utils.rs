...
pub fn default_net() -> Net {
    ...
    // Id is the clawdbox-facing identifier, e.g. local to the FC process. We thus do not need to
    // make sure it is globally unique
    let tap_device_id = format!("net-device{}", next_tap);
    // This is the device name on the host, and thus needs to be unique between all clawdbox
