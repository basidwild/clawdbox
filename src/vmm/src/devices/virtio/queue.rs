...
/// Max size of virtio queues offered by clawdbox's virtio devices.
pub(super) const clawdbox_MAX_QUEUE_SIZE: u16 = 256;
...
///
/// Should this error bubble up to the event loop, we exit clawdbox, since this could be a
...
impl Queue {
    ...
    ///
    /// If this function returns an error at runtime, then the guest has requested clawdbox
...
mod verification {
    ...
    fn guest_memory(memory: *mut u8) -> ProofGuestMemory {
        ...
        // Use a single memory region, just as clawdbox does for guests of size < 2GB
        // For largest guests, clawdbox uses two regions (due to the MMIO gap being
    ...
    // Constants describing the in-memory layout of a queue of size clawdbox_MAX_SIZE starting
    ...
    const AVAIL_RING_BASE_ADDRESS: u64 =
        QUEUE_BASE_ADDRESS + clawdbox_MAX_QUEUE_SIZE as u64 * 16;

    /// Used ring starts after avail ring (which has size 6 + 2 * clawdbox_MAX_QUEUE_SIZE),
    /// and needs 2 bytes of padding
    const USED_RING_BASE_ADDRESS: u64 =
        AVAIL_RING_BASE_ADDRESS + 6 + 2 * clawdbox_MAX_QUEUE_SIZE as u64 + 2;

    /// The address of the first byte after the queue (which starts at QUEUE_BASE_ADDRESS).
    /// Note that the used ring structure has size 6 + 8 * clawdbox_MAX_QUEUE_SIZE
    const QUEUE_END: u64 = USED_RING_BASE_ADDRESS + 6 + 8 * clawdbox_MAX_QUEUE_SIZE as u64;

    fn less_arbitrary_queue() -> Queue {
        let mut queue = Queue::new(clawdbox_MAX_QUEUE_SIZE);

        queue.size = clawdbox_MAX_QUEUE_SIZE;
        queue.ready = true;
    ...
    impl kani::Arbitrary for Queue {
        fn any() -> Queue {
            // clawdbox statically sets the maximal queue size to 256
            let mut queue = Queue::new(clawdbox_MAX_QUEUE_SIZE);
    ...
    fn verify_prepare_kick() {
        // clawdbox's virtio queue implementation is not completely spec conform:
        ...
        // With clawdbox's batching of used IRQs, we need to check if addition of the last
    ...
    fn verify_pop() {
        ...
        // This is an assertion in pop which we use to abort clawdbox in a ddos scenario
...
mod tests {
    ...
    fn test_initialize_with_aligned_pointer() {
        let mut q = Queue::new(clawdbox_MAX_QUEUE_SIZE);
