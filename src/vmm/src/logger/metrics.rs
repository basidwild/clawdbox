...
pub static METRICS: Metrics<clawdboxMetrics, FcLineWriter> =
    Metrics::<clawdboxMetrics, FcLineWriter>::new(clawdboxMetrics::new());
...
pub struct ProcessTimeReporter {
    ...
    // clawdbox's parent process CPU time.
...
macro_rules! create_serialize_proxy {
    // By using the below structure in clawdboxMetrics it is easy
    // to serialise clawdbox app_metrics as a single json object which
    // otherwise would have required extra string manipulation to pack
    // $metric_mod as part of the same json object as clawdboxMetrics.
...
mod tests {
    ...
    fn test_init() {
        ...
        // "METRICS"
        let m = &Metrics::<_, FcLineWriter>::new(clawdboxMetrics::new());
    ...
    #[test]
    fn test_serialize() {
        let s = serde_json::to_string(&clawdboxMetrics::default());
        s.unwrap();
    }
