...
use super::target::{clawdboxTarget, GdbTargetError, vcpuid_to_tid};
use crate::Vmm;
...
pub fn event_loop(
    ...
    let target = clawdboxTarget::new(vmm, gdb_event_receiver, entry_addr);
...
impl run_blocking::BlockingEventLoop for GdbBlockingEventLoop {
    ...
    fn on_interrupt(
        target: &mut clawdboxTarget,
    ) -> Result<Option<MultiThreadStopReason<u64>>, <clawdboxTarget as Target>::Error> {
...
fn gdb_event_loop_thread(
    debugger: GdbStub<clawdboxTarget, Box<dyn ConnectionExt<Error = std::io::Error>>>,
    mut target: clawdboxTarget,
) {
