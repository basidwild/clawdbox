...
//! Defines state structures for saving/restoring a clawdbox microVM.
...
/// This describes the mapping between clawdbox base virtual address and
...
fn send_uffd_handshake(
    ...
    socket.send_with_fd(
        ...
        //
        // The problem is that if other process crashes/exits, clawdbox guest memory
        // will simply revert to anon-mem behavior which would lead to silent errors and
        // undefined behavior.
        //
        // To tackle this scenario, the page fault handler can notify clawdbox of any
        // crashes/exits. There is no need for clawdbox to explicitly send its process ID.
        // The external process can obtain clawdbox's PID by calling `getsockopt` with
        ...
        //
        // Moreover, clawdbox holds a copy of the UFFD fd as well, so that even if the
        // page fault handler process does not tear down clawdbox when necessary, the
