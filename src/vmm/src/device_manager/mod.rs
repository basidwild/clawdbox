...
#[derive(Debug)]
/// A manager of all peripheral devices of clawdbox
...
impl DeviceManager {
    ...
    fn set_stdout_nonblocking() {
        ...
        if flags < 0 {
            error!("Could not get clawdbox stdout flags.");
        }
        // SAFETY: Call is safe since parameters are valid.
        let rc =
            unsafe { libc::fcntl(libc::STDOUT_FILENO, libc::F_SETFL, flags | libc::O_NONBLOCK) };
        if rc < 0 {
            error!("Could not set clawdbox stdout to non-blocking.");
        }
    }
    ...
    /// Enables PCIe support for clawdbox devices
