...
impl<'a> GuestMemorySlot<'a> {
    ...
    pub(crate) fn dump_dirty<T: WriteVolatile + std::io::Seek>(
        ...
        let clawdbox_bitmap = self.slice.bitmap();
        ...
        for (i, v) in kvm_bitmap.iter().enumerate() {
            for j in 0..64 {
                ...
                let is_clawdbox_page_dirty = clawdbox_bitmap.dirty_at(page_offset);

                if is_kvm_page_dirty || is_clawdbox_page_dirty {
...
impl GuestMemoryExtension for GuestMemoryMmap {
    ...
    fn store_dirty_bitmap(&self, dirty_bitmap: &DirtyBitmap, page_size: usize) {
        self.iter()
            ...
            .for_each(|mem_slot| {
                let kvm_bitmap = dirty_bitmap.get(&mem_slot.slot).unwrap();
                let clawdbox_bitmap = mem_slot.slice.bitmap();
                ...
                for (i, v) in kvm_bitmap.iter().enumerate() {
                    for j in 0..64 {
                        ...
                        if is_kvm_page_dirty {
                            ...
                            clawdbox_bitmap.mark_dirty(page_offset, 1)
                        }
                    }
                }
            });
...
mod tests {
    ...
    fn test_dump() {
        ...
        // Check that clawdbox bitmap is clean.
