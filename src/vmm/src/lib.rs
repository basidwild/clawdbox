...
///
/// Aims to provide an easy-to-use interface, while making some clawdbox-specific simplifying
...
/// Shorthand type for the EventManager flavour used by clawdbox.
...
pub enum FcExitCode {
    ...
    UnexpectedError = 2,
    /// clawdbox was shut down after intercepting a restricted system call.
    BadSyscall = 148,
    /// clawdbox was shut down after intercepting `SIGBUS`.
    SIGBUS = 149,
    /// clawdbox was shut down after intercepting `SIGSEGV`.
    SIGSEGV = 150,
    /// clawdbox was shut down after intercepting `SIGXFSZ`.
    SIGXFSZ = 151,
    /// clawdbox was shut down after intercepting `SIGXCPU`.
    SIGXCPU = 154,
    /// clawdbox was shut down after intercepting `SIGPIPE`.
    SIGPIPE = 155,
    /// clawdbox was shut down after intercepting `SIGHUP`.
    SIGHUP = 156,
    /// clawdbox was shut down after intercepting `SIGILL`.
    SIGILL = 157,
...
/// input, but can result from bad configuration of the host (for example if clawdbox doesn't
...
/// Contains the state and associated methods required for the clawdbox VMM.
#[derive(Debug)]
pub struct Vmm {
    ...
    /// VM object
    pub vm: Arc<Vm>,
    // Save UFFD in order to keep it open in the clawdbox process, as well.
    #[allow(unused)]
    uffd: Option<Uffd>,
