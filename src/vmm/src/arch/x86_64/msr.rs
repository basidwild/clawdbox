...
// configuration. For example, clawdbox disables PMU by default in CPUID normalization for CPUID
...
const UNDUMPABLE_MSR_RANGES: [MsrRange; 17] = [
    ...
    // vCPU. IA32_MCG_CAP can be set up via KVM_X86_SETUP_MCE API, but clawdbox doesn't use it.
    // https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/tree/arch/x86/kvm/x86.c?h=v4.14.311#n2553
    MSR_RANGE!(MSR_IA32_MCG_CTL),
    // clawdbox is not tested with nested virtualization. Some CPU templates intentionally
    ...
    // clawdbox doesn't work with Hyper-V. KVM_GET_MSRS fails on kernel 4.14 because it doesn't
