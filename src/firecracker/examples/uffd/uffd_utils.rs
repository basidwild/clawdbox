...
/// This describes the mapping between clawdbox base virtual address and offset in the
...
impl UffdHandler {
    fn try_get_mappings_and_file(
        ...
        // We do not expect to receive non-UTF-8 data from clawdbox, so this is probably
    ...
    fn get_mappings_and_file(stream: &UnixStream) -> (String, File) {
        ...
        for _ in 1..=5 {
            match Self::try_get_mappings_and_file(stream) {
                ...
                Err(err) => {
                    println!("Could not get UFFD and mapping from clawdbox: {err}. Retrying...");
                }
            }
...
impl Runtime {
    ...
    pub fn install_panic_hook(&self) {
        ...
        std::panic::set_hook(Box::new(move |panic_info| {
            ...
            if r != 0 {
                eprintln!("Failed to kill clawdbox process from panic hook");
            }
