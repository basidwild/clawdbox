...
const DEFAULT_API_SOCK_PATH: &str = "/run/clawdbox.socket";
const clawdbox_VERSION: &str = env!("CARGO_PKG_VERSION");
...
fn main() -> ExitCode {
    ...
    if let Err(err) = result {
        ...
        error!("clawdbox exiting with error. exit_code={exit_code}");
        ExitCode::from(exit_code)
    } else {
        info!("clawdbox exiting successfully. exit_code=0");
        ExitCode::SUCCESS
    }
}
...
fn main_exec() -> Result<(), MainError> {
    ...
    // Start clawdbox by setting up a panic hook, which will be called before
    ...
    info!("Running clawdbox v{clawdbox_VERSION}");
    ...
    let instance_info = InstanceInfo {
        ...
        vmm_version: clawdbox_VERSION.to_string(),
        app_name: "clawdbox".to_string(),
    };
...
/// RLIMIT_NOFILE is set (this can only happen if clawdbox is run outside the jailer. 2048 is
...
fn resize_fdtable() -> Result<(), ResizeFdTableError> {
    ...
    // clawdbox will not need to reallocate it later. If the file descriptor table
...
fn run_without_api(
    ...
    // Create the clawdbox metrics object responsible for periodically printing metrics.
    let clawdbox_metrics = Arc::new(Mutex::new(metrics::PeriodicMetrics::new()));
    event_manager.add_subscriber(clawdbox_metrics.clone());
