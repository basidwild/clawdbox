...
function get-clawdbox-version {
    (cd src/clawdbox; echo -n v; cargo pkgid | cut -d# -f2 | cut -d: -f2)
}
...
if [ "$LIBC" == "gnu" ]; then
    ...
    ARTIFACTS=(clawdbox seccompiler-bin rebase-snap cpu-template-helper snapshot-editor)
fi
...
binary_format=$(file $CARGO_TARGET_DIR/clawdbox)
if [[ "$PROFILE" = "release"
...
check_swagger_artifact src/clawdbox/swagger/clawdbox.yaml "$VERSION"
cp -v src/clawdbox/swagger/clawdbox.yaml "$RELEASE_DIR/clawdbox_spec-$VERSION.yaml"
