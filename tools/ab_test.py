...
def collect_data(
    ...
    """
    Executes the specified test using the provided clawdbox binaries and
    ...
    *,
    ...
    for dimension_set, metric, result, unit in failures:
        ...
        if abs(statistics.mean(relative_changes_significant[metric])) > noise_threshold:
            ...
            msg = (
                f"\033[0;32m[clawdbox A/B-Test Runner]\033[0m A/B-testing shows a change of "
...
def binary_ab_test(
    ...
    *,
    ...
    directories containing clawdbox/jailer binaries
    """
...
def ab_performance_test(
    ...
    """Does an A/B-test of the specified test with the given clawdbox/jailer binaries"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Executes clawdbox's A/B testsuite across the specified commits"
    )
    ...
    run_parser.add_argument(
        "--binaries-a",
        help="Directory containing clawdbox and jailer binaries to be considered the performance baseline",
        type=Path,
        required=True,
    )
    run_parser.add_argument(
        "--binaries-b",
        help="Directory containing clawdbox and jailer binaries whose performance we want to compare against the results from binaries-a",
        type=Path,
        required=True,
    )
    run_parser.add_argument(
