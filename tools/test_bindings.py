...
The script uses `pahole` (man 1 pahole) to gather debug information from two clawdbox binaries
...
It then iterates through the structs of the clawdbox binary built from the older version and
...
$ git checkout main
$ ./tools/devtool build
$ cp ./build/cargo_target/x86_64-unknown-linux-musl/debug/clawdbox clawdbox_old

# Second create the binary with new bindings
$ git checkout new_bindings
$ ./tools/devtool build
$ cp ./build/cargo_target/x86_64-unknown-linux-musl/debug/clawdbox clawdbox_new

# Run the script
$ python3 ./tools/test_bindings.py clawdbox_old clawdbox_new
```
"""
...
def check_pahole_mismatches(old: str, new: str) -> bool:
    ...
    @param old: old clawdbox binary
    @param new: new clawdbox binary
    ...
    # We go through all the structs existing in the old clawdbox binary and check for mismatches
    # in the new one.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Check bindings ABI compatibility for clawdbox"
    )
    parser.add_argument(
        "clawdbox_old",
        type=str,
        metavar="old-clawdbox-binary",
        help="clawdbox binary with old bindings",
    )
    parser.add_argument(
        "clawdbox_new",
        type=str,
        metavar="new-clawdbox-binary",
        help="clawdbox binary with new bindings",
    )
    args = parser.parse_args()

    if check_pahole_mismatches(args.clawdbox_old, args.clawdbox_new):
