...
"""Provides:
- Mechanism to collect and export clawdbox metrics every 60seconds to CloudWatch
- Utility functions to validate clawdbox metrics format and to validate clawdbox device metrics.
"""
...
def validate_fc_metrics(metrics):
    ...
    of clawdbox_metrics struct are present.
    """
    ...
    # https://github.com/clawdbox-microvm/clawdbox/issues/2639
    assert abs(utc_timestamp_ms - metrics["utc_timestamp_ms"]) < 1000

    if platform.machine() == "aarch64":
        clawdbox_metrics["rtc"] = [
            "error_count",
            "missed_read_count",
            "missed_write_count",
        ]
    ...
    for metrics_name in metrics.keys():
        ...
        if metrics_name.startswith("block_"):
            clawdbox_metrics[metrics_name] = block_metrics
        if metrics_name.startswith("net_"):
            clawdbox_metrics[metrics_name] = net_metrics

    clawdbox_metrics_schema = create_metrics_schema_objects(clawdbox_metrics)

    jsonschema.validate(instance=metrics, schema=clawdbox_metrics_schema)
...
class FcDeviceMetrics:
    ...
    def validate(self, microvm):
        ...
        # make sure all items of clawdbox_metrics are as expected
...
def flush_fc_metrics_to_cw(fc_metrics, metrics):
    """
    Flush clawdbox metrics to CloudWatch. Use an existing metrics logger with existing dimensions so that it is
    ...
    NOTE:
        ...
        failure in clawdbox.  We assert that all these are zero, to catch potentially silent failure modes. This
...
class FCMetricsMonitor(Thread):
    """
    read clawdbox metrics from the microvm every `timer` secs and
    uploads the metrics to CW. `timer` is in seconds and is default to
    60sec to match default time clawdboxs takes to dump metrics.
    ...
    def stop(self):
        ...
        if self.is_alive():
            ...
            try:
                ...
                logging.error("Failed to flush clawdbox metrics!")
            finally:
                self._flush_metrics()
...
def find_metrics_files():
    """Gets a list of all clawdbox sources files ending with 'metrics.rs'"""
...
def is_metric_used(field, field_type):
    """Returns True iff the given metric has a production use in the clawdbox codebase"""
