...
"""Track clawdbox memory overhead

Because clawdbox is a static binary, and is copied before execution, no
memory is shared across many different processes. It is thus important to track
how much memory overhead clawdbox adds.

These tests output metrics to capture the memory overhead that clawdbox adds,
both from the binary file (file-backed) and what clawdbox allocates during
...
def test_memory_overhead(
    ...
    for _ in range(5):
        ...
        ps = psutil.Process(microvm2.clawdbox_pid)
        ...
        for pmmap in ps.memory_maps(grouped=False):
            ...
            if pmmap_path.exists() and pmmap_path.name.startswith("clawdbox"):
