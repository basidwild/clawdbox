...
# Make sure we don't override the clawdbox binary used from other tests
...
def test_redundant_seccomp_rules():
    ...
    utils.check_output(
        f'CARGO_TARGET_DIR={TMP_BUILD_DIR} RUSTFLAGS="-C relocation-model=static -C link-args=-no-pie" cargo +{nightly_toolchain} -Zbuild-std=panic_abort,std build --release --target {target} -p clawdbox'
    )

    found_syscalls = find_syscalls_in_binary(
        Path(f"{TMP_BUILD_DIR}/{target}/release/clawdbox")
    )
