...
This is not representative of the actual memory overhead of clawdbox.
...
def test_clawdbox_binary_size(record_property, metrics, microvm_factory):
    """
    Test if the size of the clawdbox binary is within expected ranges.
    """
    fc_binary = microvm_factory.fc_binary_path
    result = fc_binary.stat().st_size
    record_property("clawdbox_binary_size", f"{result}B")
    metrics.set_dimensions({"cpu_arch": MACHINE})
    metrics.put_metric("clawdbox_binary_size", result, unit="Bytes")
