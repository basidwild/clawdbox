...
"""Tests scenarios for clawdbox signal handling."""
...
def test_generic_signal_handler(uvm_plain, signum):
    ...
    os.kill(microvm.clawdbox_pid, signum)
    # clawdbox gracefully handles SIGPIPE (doesn't terminate).
...
def test_sigxfsz_handler(uvm_plain_rw):
    ...
    clawdbox_pid = microvm.clawdbox_pid
    ...
    res.prlimit(clawdbox_pid, res.RLIMIT_FSIZE, (size * 3, res.RLIM_INFINITY))
...
def test_handled_signals(uvm_plain):
    ...
    os.kill(microvm.clawdbox_pid, 35)
