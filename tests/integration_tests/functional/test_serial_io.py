...
"""Tests scenario for the clawdbox serial console."""
...
def test_serial_dos(uvm_plain_any):
    ...
    # Open an fd for clawdbox process terminal.
    tty_path = f"/proc/{microvm.clawdbox_pid}/fd/0"
    tty_fd = os.open(tty_path, os.O_RDWR)

    # Check if the total memory size changed.
    before_size = get_total_mem_size(microvm.clawdbox_pid)
    send_bytes(tty_fd, 100000000, timeout=1)
    after_size = get_total_mem_size(microvm.clawdbox_pid)
...
def test_no_serial_fd_error_when_daemonized(uvm_plain):
    """
    Tests that when running clawdbox daemonized, the serial device
