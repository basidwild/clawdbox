...
def test_unbinded_socket(uvm_plain, snapshot):
    ...
    jailed_vmstate = vm.create_jailed_resource(snapshot.vmstate)
    socket_path = os.path.join(vm.path, "clawdbox-uffd.sock")
...
def test_malicious_handler(uvm_plain, snapshot):
    ...
    loaded into memory. In this case, clawdbox is designed to freeze,
    ...
    # We expect clawdbox to freeze while resuming from a snapshot
    ...
    try:
        with Timeout(seconds=30):
            ...
            assert False, "clawdbox should freeze"
