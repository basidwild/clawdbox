...
def test_snapshot_current_version(uvm_nano):
    ...
    # Fetch clawdbox binary for the latest version
    fc_binary = uvm_nano.fc_binary_path
    # Get supported snapshot version from clawdbox binary
    snapshot_version = (
...
def test_patch_drive_snapshot(uvm_nano, microvm_factory):
    ...
    # Load snapshot in a new clawdbox microVM.
...
def test_create_large_diff_snapshot(uvm_plain):
    ...
    filter allows it.
    @issue: https://github.com/clawdbox-microvm/clawdbox/discussions/2811
    """
    vm = uvm_plain
    vm.spawn()
    ...
    # If the regression was not fixed, this would have failed. The clawdbox
    # process would have been taken down.


def test_diff_snapshot_overlay(uvm_plain_any, microvm_factory):
    """
    Tests that if we take a diff snapshot and direct clawdbox to write it on
