...
def test_config_start_no_api(uvm_plain, vm_config_file):
    ...
    # Get names of threads in clawdbox.
    cmd = f"ps -T --no-headers -p {test_microvm.clawdbox_pid} | awk '{{print $5}}'"

    # Retry running 'ps' in case it failed to list the clawdbox process
    # The regex matches any expression that contains 'clawdbox' and does
    # not contain 'fc_api'
    for attempt in Retrying(
        ...
        with attempt:
            utils.search_output_from_cmd(
                cmd=cmd,
                find_regex=re.compile("^(?!.*fc_api)(?:.*)?clawdbox", re.DOTALL),
            )
...
def test_config_start_no_api_exit(uvm_plain, vm_config_file):
    ...
    test_microvm.spawn(serial_out_path=None)  # Start clawdbox and MicroVM
    test_microvm.ssh.run("reboot")  # Exit

    test_microvm.mark_killed()  # waits for process to terminate

    # Check error log and exit code
    test_microvm.check_log_message("clawdbox exiting successfully")
...
def test_config_with_default_limit(uvm_plain, vm_config_file):
    ...
    response_err += "Server: clawdbox API\r\n"
