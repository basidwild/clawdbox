...
"""Tests that ensure the correctness of the clawdbox API."""
...
from framework.utils import get_clawdbox_version_from_toml
...
def test_api_machine_config(uvm_plain):
    ...
    clawdbox_pid = test_microvm.clawdbox_pid
    resource.prlimit(
        clawdbox_pid, resource.RLIMIT_AS, (MEM_LIMIT, resource.RLIM_INFINITY)
    )
...
def test_api_version(uvm_plain):
    ...
    assert "clawdbox_version" in preboot_response.json()
    ...
    # Check that the response contains the version.
    assert "clawdbox_version" in postboot_response.json()
    # Validate VM version post-boot is the same as pre-boot.
    assert preboot_response.json() == postboot_response.json()

    cargo_version = get_clawdbox_version_from_toml()
    api_version = semver.Version.parse(preboot_response.json()["clawdbox_version"])

    # Cargo version should match FC API version
    assert cargo_version == api_version

    binary_version = semver.Version.parse(test_microvm.clawdbox_version)
