...
"""
Integration test verifying that clawdbox builds and runs correctly when instrumented
...
PATHS_TO_INSTRUMENT = [
    "../src/clawdbox/src/main.rs",
    "../src/clawdbox/src/api_server",
    "../src/vmm/src/lib.rs",
    "../src/vmm/src/builder.rs",
]
TMP_BUILD_DIR = "../test_instrumented_clawdbox_build"
ARCH_STR = f"{platform.machine()}"


def build_instrumented_binary():
    """Builds an instrumented clawdbox binary with tracing instrumentation."""
    ...
    cargo(
        ...
        f"--features tracing --bin clawdbox",
        env={"CARGO_TARGET_DIR": TMP_BUILD_DIR},
    )
    return get_binary("clawdbox", binary_dir=instrumented_binary_dir)


def cleanup_instrumentation():
    """Cleans up tracing instrumentation from the clawdbox binary."""
    clippy_tracing = get_binary("clippy-tracing")
    for p in PATHS_TO_INSTRUMENT:
        utils.check_output(
...
def instrumented_binary():
    """Build and provide the path to an instrumented clawdbox binary."""
    binary_path = build_instrumented_binary()
    yield binary_path
    cleanup_instrumentation()


def test_log_instrument_clawdbox_basic_functionality(
    instrumented_binary, microvm_factory
):
    """Test that instrumented clawdbox can start and handle basic API calls with trace logging."""
