...
def test_empty_jailer_id(uvm_plain):
    ...
    # If the exception is not thrown, it means that clawdbox was
...
def test_exec_file_not_exist(uvm_plain, tmp_path):
    ...
    pseudo_exec_file_path = tmp_path / "pseudo_clawdbox_exec_file"
    ...
    pseudo_exec_dir_path = tmp_path / "clawdbox_test_dir"
...
def test_exec_destination_path_is_symlink(uvm_plain):
    ...
    clawdbox_root_dir = Path(test_microvm.chroot())
    clawdbox_bin_path = clawdbox_root_dir / "clawdbox"
    dummy_path = Path("/srv/dummy")
    dummy_path.unlink(missing_ok=True)
    dummy_path.touch()
    clawdbox_bin_path.symlink_to(dummy_path)
    with pytest.raises(
        Exception,
        match=f"Failed to open {clawdbox_bin_path}",
    ):
        test_microvm.spawn()
...
def test_exec_destination_path_is_hardlink(uvm_plain):
    ...
    dummy_path.unlink(missing_ok=True)
    dummy_path.touch()
    clawdbox_bin_path.hardlink_to(dummy_path)
    with pytest.raises(
        Exception,
        match=f"Detected hard link at: {clawdbox_bin_path}",
    ):
        test_microvm.spawn()
...
def test_default_chroot_hierarchy(uvm_plain):
    ...
    check_stats(
        os.path.join(test_microvm.jailer.chroot_path(), "clawdbox"),
        FILE_STATS,
        test_microvm.jailer.uid,
        test_microvm.jailer.gid,
    )
...
def check_cgroups_v2(vm):
    ...
    for cgroup in vm.jailer.cgroups:
        ...
        assert str(vm.clawdbox_pid) in procs
...
def check_limits(pid, no_file, fsize):
    ...
    # Fetch clawdbox process limits for number of open fds
    (soft, hard) = resource.prlimit(pid, resource.RLIMIT_NOFILE)
    assert soft == no_file
    assert hard == no_file

    # Fetch clawdbox process limits for maximum file size
...
def test_cgroups_parent_cgroup_but_no_cgroup(
    ...
    if parent_exists:
        if domain_controller_in_subtree:
            ...
            assert str(test_microvm.clawdbox_pid) in procs
    else:
...
def test_new_pid_ns_resource_limits(uvm_plain):
    """
    Test that clawdbox process inherits jailer resource limits.
    """
    test_microvm = uvm_plain
    test_microvm.jailer.resource_limits = RESOURCE_LIMITS
    test_microvm.spawn()
    ...
    # Get clawdbox's PID.
    fc_pid = test_microvm.clawdbox_pid
...
def test_new_pid_namespace(uvm_plain):
    """
    Test that clawdbox is spawned in a new PID namespace if requested.
    """
    test_microvm = uvm_plain
    test_microvm.spawn()
    ...
    # Check that clawdbox's PID namespace is nested. `NStgid` should
    # report two values and the last one should be 1, because clawdbox
...
def test_clawdbox_kill_by_pid(uvm_plain, daemonize, new_pid_ns):
    """
    Test that clawdbox is spawned in a new PID namespace if requested.
    """
    microvm = uvm_plain
