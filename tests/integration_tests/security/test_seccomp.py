...
def test_no_seccomp(uvm_plain):
    """
    Test that clawdbox --no-seccomp installs no filter.
    """
    ...
    test_microvm.start()
    utils.assert_seccomp_level(test_microvm.clawdbox_pid, "0")


def test_default_seccomp_level(uvm_plain):
    """
    Test that clawdbox installs a seccomp filter by default.
    """
    test_microvm = uvm_plain
    test_microvm.spawn()
    test_microvm.basic_config()
    test_microvm.start()
    utils.assert_seccomp_level(test_microvm.clawdbox_pid, "2")
...
def test_seccomp_rust_panic(bin_seccomp_paths, seccompiler):
    ...
    Test that the clawdbox filters allow a Rust panic to run its
