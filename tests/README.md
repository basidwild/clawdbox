...
The `pytest`-powered integration tests rely on clawdbox's HTTP API for
...
generated by running the test function in environment _A_. clawdbox's
A/B-testing generally compares clawdbox binaries compiled from two separate
...
clawdbox has some functional A/B-tests (for example, in
...
clawdbox has a special framework for orchestrating long-running A/B-tests
...
different clawdbox binaries, and then matches up corresponding data series
...
- _dir A_ and _dir B_ are directories containing clawdbox and jailer binaries
...
compare boottime of microVMs between clawdbox binaries compiled from the
...
performance differences that are present in the clawdbox binary. The
`tools/ab_test.py` script only checks out the revisions it is passed to execute
`cargo build` to generate a clawdbox binary. It does not run integration
...
arbitrary environment (such as comparison how the same clawdbox binary
...
Traceback (most recent call last):
  File "/clawdbox/tools/ab_test.py", line 412, in <module>
    data_a = load_data_series(args.report_a)
  File "/clawdbox/tools/ab_test.py", line 122, in load_data_series
...
of clawdbox processes. The following fixtures spawn clawdbox processes
that are pre-initialized with specific guest kernels and rootfs:

- `uvm_plain_any` is parametrized by the guest kernels
  [supported](../docs/kernel-policy.md) by clawdbox and a read-only Ubuntu
  24.04 squashfs as rootfs,
- `uvm_plain` yields a clawdbox process pre-initialized with a 5.10 kernel
...
between the guest and clawdbox, and `uvm_plain` should be used if clawdbox
functionality unrelated to the guest is being tested.

### Markers

clawdbox uses two special
...
# Get clawdbox
yum install -y git
git clone https://github.com/clawdbox-microvm/clawdbox.git
...
- **Test**: A function named `test_` from this tree, that ensures a feature,
  functional parameter, or quality metric of clawdbox. Should assert or raise
  an exception if it fails.
- **Fixture**: A function that returns an object that makes it very easy to add
  **Tests**: E.g., a spawned clawdbox microvm. Fixtures are functions marked
...
`Q2:` *I want to add more tests that I don't want to commit to the clawdbox
repository.*\
...
clawdbox source tree. This directory is bind-mounted in the container and
...
## How to run tests with a different version of clawdbox

The integration tests usually compile clawdbox as part of the test
initialization. But there's an option in case we want to run the tests against a
different version of clawdbox, for example a previous release:
