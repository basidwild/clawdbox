...
def get_clawdbox_version_from_toml():
    """
    Return the version of the clawdbox crate, from Cargo.toml.

    Should be the same as the output of `./clawdbox --version`, if
    the code has not been released.
    """
    cmd = "cd ../src/clawdbox && cargo pkgid | cut -d# -f2 | cut -d: -f2"
    _, stdout, _ = check_output(cmd)
