...
- Use the clawdbox Open API spec to populate Microvm API resource URLs.
"""
...
class SnapshotType(Enum):
    ...
    def api_type(self) -> str:
        """Converts this `SnapshotType` to the string value expected by the clawdbox API"""
        match self:
...
class HugePagesConfig(str, Enum):
    """Enum describing the huge pages configurations supported clawdbox"""
...
class Microvm:
    """Class to represent a clawdbox microvm.
    ...
    def mark_killed(self):
        ...
        raises an exception if the clawdbox process managing this VM is not actually dead
        """
        if self.clawdbox_pid is not None:
            utils.wait_process_termination(self.clawdbox_pid)
    ...
    def kill(self, might_be_dead=False):
        ...
        # because if we stop clawdbox first, the backend will want
        ...
        try:
            if self.clawdbox_pid:
                os.kill(self.clawdbox_pid, signal.SIGKILL)
            ...
            if not might_be_dead:
                msg = (
                    "Failed to kill clawdbox Process. Did it already die (or did the UFFD handler process die and take it down)?"
                    if self.uffd_handler
                    else "Failed to kill clawdbox Process. Did it already die?"
                )
        ...
        if self._spawned:
            # Wait until the clawdbox process is actually dead
            utils.wait_process_termination(self.clawdbox_pid)

            # The following logic guards us against the case where `clawdbox_pid` for some
            # reason is the wrong PID, e.g. this is a regression test for
            # https://github.com/clawdbox-microvm/clawdbox/pull/4442/commits/d63eb7a65ffaaae0409d15ed55d99ecbd29bc572
            ...
            for proc in stdout.splitlines():
                ...
                if "clawdbox" in proc and not cmd.startswith("screen"):
                    offenders.append(proc)

            # make sure clawdbox was killed
            assert not offenders, (
                f"clawdbox reported its pid {self.clawdbox_pid}, which was killed, but there still exist processes using the supposedly dead clawdbox's jailer_id: \n"
                + "\n".join(offenders)
            )
    ...
    def _validate_api_response_times(self):
        """
        Parses the clawdbox logs for information regarding api server request processing times, and asserts they
    ...
    @property
    def clawdbox_version(self):
        """Return the version of the clawdbox executable."""
        _, stdout, _ = utils.check_output(f"{self.fc_binary_path} --version")
        return re.match(r"^clawdbox v(.+)", stdout.partition("\n")[0]).group(1)
    ...
    @cached_property
    def clawdbox_pid(self):
        """Return clawdbox's PID
        ...
        # Read the PID from clawdbox's pidfile. Retry if
    ...
    def ps(self):
        ...
        return psutil.Process(self.clawdbox_pid)
    ...
    def pin_vmm(self, cpu_id: int) -> bool:
        """Pin the clawdbox process VMM thread to a cpu list."""
        if self.clawdbox_pid:
            for thread_name, thread_pids in utils.get_threads(
                self.clawdbox_pid
            ).items():
                # the clawdbox thread should start with clawdbox...
                if thread_name.startswith("clawdbox"):
    ...
    def pin_vcpu(self, vcpu_id: int, cpu_id: int):
        """Pin the clawdbox vcpu thread to a cpu list."""
        if self.clawdbox_pid:
            for thread in utils.get_threads(self.clawdbox_pid)[f"fc_vcpu {vcpu_id}"]:
                utils.set_cpu_affinity(thread, [cpu_id])
            return True
        return False

    def pin_api(self, cpu_id: int):
        """Pin the clawdbox process API server thread to a cpu list."""
        if self.clawdbox_pid:
            for thread in utils.get_threads(self.clawdbox_pid)["fc_api"]:
                utils.set_cpu_affinity(thread, [cpu_id])
            return True
        return False
    ...
    def spawn(
        ...
        if self.jailer.daemonize:
            ...
            # Run clawdbox under screen. This is used when we want to access
        ...
        # If `--new-pid-ns` is used, the clawdbox process will detach from
        ...
        # Ensure clawdbox is in as good a state as possible wrts guest
        ...
        # clawdbox process itself at least came up by checking
        ...
        if "config-file" in self.jailer.extra_args and self.iface:
            ...
            assert not serial_out_path
            self.check_log_message("Running clawdbox")
    ...
    def get_exit_code(self):
        ...
        exit_msg_pattern = (
            r"clawdbox exiting (with error|successfully). exit_code=(\d+)"
        )
    ...
    def serial_input(self, input_string):
        """Send a string to the clawdbox serial console via screen."""
    ...
    def basic_config(
        ...
        which differs from clawdbox's default only in the enabling of the serial console.
    ...
    def thread_backtraces(self):
        ...
        backtraces = []
        for thread_name, thread_pids in utils.get_threads(self.clawdbox_pid).items():
            for pid in thread_pids:
                try:
...
class MicroVMFactory:
    ...
    def __init__(self, binary_path: Path, **kwargs):
        ...
        assert self.fc_binary_path.exists(), "missing clawdbox binary"
        assert self.jailer_binary_path.exists(), "missing jailer binary"

    @property
    def fc_binary_path(self):
        """The path to the clawdbox binary from which this factory will build VMs"""
        return self.binary_path / "clawdbox"
