...
class MicrovmHelpers:
    ...
    def gdbserver(self, port=2000):
        ...
        subprocess.Popen(["gdbserver", "--attach", comm, str(self.vm.clawdbox_pid)])
    ...
    def lldbserver(self, port=2001):
        ...
        print(
            f"Connect lldb with\n\tlldb -o 'platform select remote-linux' -o 'platform connect connect://{DOCKER.ip}:{port}' -o 'attach {self.vm.clawdbox_pid}'"
        )
